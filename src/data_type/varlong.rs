...
pub trait FromVarLong {
    fn from_varlong(&mut self) -> Result<i64>;
}
...
impl FromVarLong for Vec<u8> {
    fn from_varlong(&mut self) -> Result<i64> {
        ...
        if self.len() == 0 {
            return Err(anyhow!("No data to parse VarLong"));
        }

        loop {
            if self.len() == current_byte_position {
                return Err(anyhow!("Not enough data to parse VarLong"));
            }
            ...
            if position >= 64 {
                return Err(anyhow!("VarLong is too big"));
            }
        }
...
mod tests {
    ...
    fn from_varlong() {
        ...
        assert_eq!(
            vec![].from_varlong().unwrap_err().to_string(),
            anyhow!("No data to parse VarLong").to_string()
        );
        assert_eq!(
            vec![0x80, 0x80].from_varlong().unwrap_err().to_string(),
            anyhow!("Not enough data to parse VarLong").to_string()
        );

        for (mut input, output) in test_data {
            assert_eq!(input.from_varlong().unwrap(), output);
