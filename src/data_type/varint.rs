...
pub trait FromVarInt {
    fn from_varint(&mut self) -> Result<i32>;
}
...
impl FromVarInt for Vec<u8> {
    fn from_varint(&mut self) -> Result<i32> {
        ...
        if self.len() == 0 {
            return Err(anyhow!("No data to parse VarInt"));
        }

        loop {
            if self.len() == current_byte_position {
                return Err(anyhow!("Not enough data to parse VarInt"));
            }
...
mod tests {
    ...
    fn from_varint() {
        ...
        assert_eq!(
            vec![].from_varint().unwrap_err().to_string(),
            anyhow!("No data to parse VarInt").to_string()
        );
        assert_eq!(
            vec![0x80, 0x80].from_varint().unwrap_err().to_string(),
            anyhow!("Not enough data to parse VarInt").to_string()
        );

        for (mut input, output) in test_data {
            assert_eq!(input.from_varint().unwrap(), output);
